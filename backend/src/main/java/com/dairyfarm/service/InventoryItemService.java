...
import com.dairyfarm.dto.InventoryItemDTO;
import com.dairyfarm.dto.InventoryUsageDTO;
...
import com.dairyfarm.repository.InventoryUsageRepository;
...
public class InventoryItemService {
    ...
    public List<InventoryUsageDTO> getUsageHistory(Long itemId) {
        ...
        return usageRepository.findByInventoryItemOrderByUsageDateDesc(item).stream()
                .map(usage -> {
    ...
    private void checkLowStock(InventoryItem item) {
        if (item.isLowStock()) {
            ...
            notification.setNotificationType(NotificationQueue.NotificationType.CUSTOM_MESSAGE);
            notification.setChannel(NotificationQueue.Channel.EMAIL);
            notification.setSubject("Low Stock Alert: " + item.getName());
            notification.setMessage("Inventory item '" + item.getName() + "' is running low. Current quantity: " + item.getQuantity() + " " + item.getUnit());
            notification.setRecipient("admin@dairyfarm.com");
